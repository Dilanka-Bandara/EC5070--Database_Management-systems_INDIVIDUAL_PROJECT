...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_attendance'])) {
    ...
    try {
        ...
        // Get attendance details before deletion and verify instructor access
        $getAttendanceStmt = $pdo->prepare("
            ...
            WHERE a.AttendanceID = ? AND ls.InstructorID = ?
        ");
        $getAttendanceStmt->execute([$attendanceID, $instructorID]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_attendance'])) {
    ...
    try {
        ...
        // Verify the attendance belongs to instructor's schedule
        $verifyStmt = $pdo->prepare("
            SELECT a.* FROM Attendance a
            JOIN LabSchedule ls ON a.ScheduleID = ls.ScheduleID
            WHERE a.AttendanceID = ? AND ls.InstructorID = ?
        ");
        $verifyStmt->execute([$attendanceID, $instructorID]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_attendance'])) {
    ...
    try {
        ...
        // Verify the schedule belongs to this instructor
        $verifyScheduleStmt = $pdo->prepare("SELECT COUNT(*) FROM LabSchedule WHERE ScheduleID = ? AND InstructorID = ?");
        $verifyScheduleStmt->execute([$scheduleID, $instructorID]);
        
        if ($verifyScheduleStmt->fetchColumn() > 0) {
            ...
            $message = "<div class='alert alert-danger'><i class='fas fa-exclamation-triangle me-2'></i>You don't have permission to add attendance for this schedule.</div>";
        }
            ...
            <div class="page-header fade-in">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <p class="page-subtitle">View, edit, and delete attendance records for your lab sessions</p>
                    </div>
                    <div class="col-md-4 text-end">
                        <div class="btn-group">
                            <a href="instructor_attendance.php" class="btn btn-outline-success">
                            ...
                            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addAttendanceModal">
                                <i class="fas fa-user-plus me-2"></i>Add Record
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card fade-in">
                        ...
                        <div class="card-body">
                            <form method="POST">
                                <div class="mb-3">
                                    <label class="form-label">Your Lab Schedules</label>
                    ...
                    <div class="card fade-in">
                        <div class="card-body text-center py-5">
                            ...
                            <div class="btn-group">
                                ...
                                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addAttendanceModal">
                                    <i class="fas fa-user-plus me-2"></i>Add Individual Record
                                </button>
                            </div>
                        </div>
                    </div>
